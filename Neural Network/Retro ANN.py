...
Training = np.load("TrainingData_IMG.npy",allow_pickle=True)
Testing = np.load("TestingData_IMG.npy",allow_pickle=True)
validation = np.load("ValidationData_IMG.npy",allow_pickle=True)
...
ytrain = ytrain/255#Normalize image
...
ytest = ytest/255#Normalize image
...
yvalidation = yvalidation/255#Normalize image
...
history=model.fit(train_dataset, epochs=150, batch_size=50,verbose=2,validation_data=(validation_dataset))
#loss, accuracy = model.evaluate(test_dataset)
#print('Accuracy: %.2f' % (accuracy*100),'Loss: %.2f' % (loss))
#dot_img_file = '/tmp/model_1.png'
model.summary()
#tf.keras.utils.plot_model(model, to_file=dot_img_file, show_shapes=True)
tf.keras.models.save_model(model,'Keras_Models/softmax_test_long_IMG')
